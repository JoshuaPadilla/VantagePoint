...
const rfReasons = [
	"Handles mixed categorical inputs and engineered numeric features without heavy scaling requirements",
	"Captures nonlinear interactions between academic history, coaching, and socio-economic factors",
	"Binary grouping with SMOTE improves separation between higher-performing and support-needed students",
	"Feature engineering with academic sum and product strengthens prior-grade signal",
];

const objectives = [
	"To collect and prepare a structured dataset related to student entrance exam performance",
	"To build a binary classification model that groups adjacent performance levels into clearer decision bands",
	"To evaluate model performance using accuracy and a confusion matrix for the grouped outcome bands",
...
const scopeItems = [
	...
	"Focused on binary classification-based prediction",
	"Groups the original four performance labels into two actionable decision bands",
	"Includes UI for prediction input and output display",
];

const contributions = [
	"Assist in predicting entrance exam outcome bands",
	"Identify patterns in academic and socio-economic factors",
	"Support earlier academic intervention for students likely to need more help",
];
...
function About() {
	...
	const groupedMappings = Object.entries(data.target_mapping ?? {}).map(
	...
	const summaryCards = [
		...
		},
		{
			...
			value: `${correctPredictions}/${totalSamples}`,
		},
		{
			label: "Test Samples",
			value: `${totalSamples}`,
		...
		<main className="page-wrap flex-1 px-4 py-8 sm:py-12 rise-in">
			<section className="mb-14 text-center max-w-3xl mx-auto">
				...
				<h1 className="mb-4 mt-0 text-2xl font-black leading-tight text-slate-900 dark:text-white sm:text-3xl md:text-4xl">
					Prediction of Student Performance Bands in the Entrance
					Examination using Binary Classification
				</h1>
				<p className="text-slate-500 dark:text-slate-400 text-base leading-relaxed m-0">
					...
					entrance examination using a binary machine learning
					classifier. The system analyzes student background data and
					academic history to determine whether a student is more
					likely to fall in the Excellent/Vg band or the Good/Average
					band.
				</p>
			</section>
			...
			<section className="mb-14 flex flex-wrap justify-center gap-3">
				{[
					...
					<span
						key={band.label}
						className={`px-5 py-2 rounded-full border text-sm font-semibold ${band.color}`}
					>
						{band.label}
					</span>
				))}
			...
			<section className="mb-14">
				<div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
					...
					<div className="flex flex-col items-start gap-6 p-5 sm:p-8 md:flex-row md:gap-8">
						...
						<div className="shrink-0 flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-slate-700 p-6 gap-2 min-w-35 text-center">
					...
					<div className="grid grid-cols-1 gap-6 p-5 sm:p-8 md:grid-cols-2 md:gap-8">
						...
						<div className="space-y-4">
							...
							<p className="text-sm text-slate-600 dark:text-slate-400 m-0">
								Binary entrance examination performance band:
							</p>
							<div className="space-y-2">
								{[
									...
									>
										...
										<span
											className={`text-sm font-semibold px-3 py-0.5 rounded-full ${target.color}`}
										>
											{target.label}
										</span>
									</div>
			...
			<section className="mb-14">
				<div className="mb-6">
					...
					<p className="text-slate-500 dark:text-slate-400 text-sm m-0">
						Current binary Random Forest model performance on the
						held-out test set from {data.dataset_source}.
					</p>
				</div>
				...
				<div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
					...
					<div className="overflow-x-auto p-4 sm:p-6">
						...
						<p className="text-xs text-slate-400 dark:text-slate-600 mt-4 mb-0">
							The grouped binary target reduces ambiguity between
							adjacent score bands and currently yields{" "}
							{(data.metrics.accuracy * 100).toFixed(2)}% accuracy
							on the held-out set.
						</p>
					</div>
				</div>
